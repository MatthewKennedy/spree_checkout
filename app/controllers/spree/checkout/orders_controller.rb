  ...
  module Checkout
    class OrdersController < Spree::Checkout::BaseController
      ...
      def update
        if @order.update_from_params(params, permitted_checkout_attributes, request.headers.env)
          ...
          if @order.completed?
            ...
            redirect_to helpers.spree_checkout_completion_route(@order)
          else
      ...
      def ensure_checkout_allowed
        redirect_to helpers.spree_checkout_cart_route unless @order.checkout_allowed?
      end

      def ensure_order_not_completed
        redirect_to helpers.spree_checkout_cart_route if @order.completed?
      end

      def ensure_sufficient_stock_lines
        if @order.insufficient_stock_lines.present?
          flash[:error] = Spree.t(:inventory_error_flash_for_insufficient_quantity)
          redirect_to helpers.spree_checkout_cart_route
        end
      end
